...
产品工作台登录页右上角可在“浅色”“深色”“跟随系统”之间切换；选择保存在当前浏览器中，登录后工作台继续使用同一主题。旁边的“界面语言”按钮可独立切换中英文，不影响主题设置。
...
正式账号第一次进入与角色对应的页面时，Web 会自动启动一次交互式新手导览。导览使用聚光蒙层突出当前控件，并同时提供进度、上一步、下一步和跳过操作；需要实际操作的步骤可以直接点击被高亮的真实控件继续。

| 使用人群 | 自动出现的位置 | 引导重点 |
| --- | --- | --- |
| 普通用户 `member` | 首次进入任一工作台页面 | 搜索与首页工作区、选股、持仓、AI 建议、回测、告警、用量、可访问的系统设置、AI 问股 |
| 审计员 `auditor` | 首次进入管理后台 `/admin` | 理解只读边界、定位审计日志、刷新审计数据 |
| 平台管理员 `platform_admin` | 首次进入管理后台 `/admin` | 查看用户、为邀请选择角色、检查审计日志 |
| 平台所有者 `platform_owner` | 首次进入管理后台 `/admin` | 使用与平台管理员相同的后台导览 |

完成或跳过导览都表示当前账号已经看过；在同一浏览器中，该账号第二次及后续登录不会再自动弹出。记忆按“账号 + 角色 + 工作台/后台入口”保存在浏览器本地，因此换用浏览器或设备、清理站点数据后，导览会再次出现。游客模式不自动显示正式账号导览。

普通用户点击“下一步”时，导览会直接进入对应功能页面并定位该模块的核心区域；如果选股能力或系统设置因配置、权限而不可用，导览会说明目标当前未显示并允许继续。导览仅演示安全操作，不会自动提交分析、创建告警、执行回测或保存系统配置。

需要重看时无需清理浏览器数据：

- 普通用户在工作台打开“更多”，点击“新手导览”。
- 审计员、平台管理员和 Owner 在管理后台页头点击“新手导览”。

手动重看不会改变权限，也不会导致下一次登录重新自动弹出。
