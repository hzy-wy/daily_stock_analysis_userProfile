...
| `/api/v1/backtest/run/async` | POST | Submit a background backtest and immediately return its task ID (the Web default) |
| `/api/v1/backtest/run/tasks/{task_id}` | GET | Query background backtest status and its completed result |
...
> Note: The Web client uses `POST /api/v1/backtest/run/async` so a long backtest does not occupy a regular HTTP request, then polls `GET /api/v1/backtest/run/tasks/{task_id}`. Repeating the same stock scope for one user returns 409 with the existing `task_id`; a full task queue returns 429 with `Retry-After: 30`. The synchronous `/run` endpoint remains available for compatibility.
