...
- `GENERATION_BACKEND_TIMEOUT_SECONDS` is the total wall-clock budget for one complete generation, capped at `3600` seconds. LiteLLM streaming, same-model non-stream fallback, fallback models, and report-integrity retries share one budget. A hard timeout returns control even if a third-party transport ignores its own timeout, so later scheduled runs are not blocked. The same value is also the local CLI execution limit.
- Other local CLI execution caps are `GENERATION_BACKEND_MAX_OUTPUT_BYTES` max `33554432`, `GENERATION_BACKEND_MAX_CONCURRENCY` max `16`, and `LOCAL_CLI_BACKEND_MAX_CONCURRENCY` max `4`. Diagnostic stdout/stderr plus the final response are counted together; for `--output-last-message` presets, the final response duplicated to stdout is not counted twice and is not exposed in `stdout_preview`.
