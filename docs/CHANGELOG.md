...
## [Unreleased]
- [新功能] Web 游客模式接入与普通用户一致的新手导览：首次进入时自动展示，并可从桌面端“更多”菜单或移动端抽屉手动重看；游客完成状态使用独立浏览器存储标识，不会写入或覆盖正式账号的导览记录。
- [改进] 回测页面改为提交后台任务并轮询状态，同用户同参数范围请求自动去重，离开页面时取消浏览器侧轮询；任务队列增加 100 个活跃任务的原子背压、256 条任务 SSE 缓冲上限和 128 条 Agent 流缓冲上限，慢客户端按协议重新同步或丢弃旧增量并保留终态，避免长回测、突发提交或断流消费者拖垮服务响应。
- [修复] 多用户鉴权复用权限初始化结果并将同步数据库操作移出事件循环；聊天会话和导入解析避免阻塞其他请求；任务 SSE 在发送快照前订阅，防止遗漏完成事件；首页隐藏时暂停定时刷新，并合并未完成的后台刷新。
- [修复] 热点刷新约 50ms 后即转后台并由页面自动查询；同进程合并重复刷新，列表默认不再附带题材详情，独立备用来源改为并行取最快有效结果，板块异动等待上限缩短至 3 秒，后台更新不再锁住刷新按钮，失败保留旧数据与原始更新时间。
- [修复] 当模型截断 JSON 但遗漏 `analysis_summary` 时，不再以“分析完成”冒充有效核心洞察；缺项会继续走完整性补全，最终仍缺失时仅用本次已采集的趋势、量价、风险、估值和数据质量快照生成带证据边界与纪律的方法论摘要。历史报告遇到旧占位摘要也会从保存快照即时重建该证据链。
- [修复] Web/API 个股分析任务只复用当天已有的大盘环境，不再在后台隐式启动完整大盘复盘并占用其锁，避免“分析全部”期间用户点击“大盘复盘”被误判为重复任务；模型生成默认并发由 1 提升为保守的 2，已确认无首个流式分片的模型会短暂直走等价非流式路径，截断 JSON 则先进行安全修复再按原有完整性补全，缩短批量分析等待而不移除模型回退、完整性校验或占位降级。
- [改进] 产品工作台登录页右上角新增“浅色 / 深色 / 跟随系统”三态主题菜单，登录前即可切换并把浏览器中的选择延续到登录后的工作台。
- [新功能] Web 新增按普通用户、审计员和平台管理员权限分流的首次交互式新手导览：以聚光蒙层高亮真实控件并支持点击推进、进度、上一步和跳过；普通用户导览可跨路由覆盖首页、选股、持仓、AI 建议、回测、告警、用量及有权限时的系统设置；完成或跳过后按浏览器内账号、角色及工作台/后台入口记忆，后续登录不再自动打扰，并保留手动重看入口。
- [改进] 新手导览移除全屏实时模糊和页面属性级监听，改为点击当帧切换内容与目标、仅用可中断短弹簧跟随位置，并在跨页面时自动定位目标，降低高分辨率页面的切步延迟。
- [修复] 修复首页存在分析任务时切换历史或今日股票缺少即时反馈的问题：点击后立即更新列表选中态并显示目标报告加载状态，旧详情请求不再覆盖后续选择。
- [改进] 登录页将通用连线粒子背景替换为股票主题行情画布，并把短周期重播改为约一小时的连续 K 线窗口与 36 秒往返均线脉冲；补充 MA21、AI 预测区间、新闻/信号节点、量化分析流水线、慢速扫描带和市场宽度等指标，同时限制为 30 FPS、页面隐藏时暂停并适配减少动态效果偏好。
...
- [修复] 首页全球市场看板将指数、市场宽度、行业和概念数据改为带总时限的分块并发刷新，并增加单分块 single-flight、最近成功缓存、结构化复盘历史预热、陈旧状态和实际覆盖字段；单一慢源不再拖到前端超时或清空整页，服务重启后也可显式标旧回退，轮询/聚焦/手动刷新不会叠加同一市场请求。
- [新功能] Web 首页新增独立且默认打开的“看板”工作区，即使已有历史报告也可从左侧“看板 / 历史 / 自选 / 今日”明确切换；看板默认 A 股、可切换中国香港/美国/日本/韩国，集中展示主要指数、市场宽度、板块热力、市场信号灯、数据覆盖状态和对应市场复盘入口，并收紧页面内容宽度以减少无效留白。
- [新功能] Web 持仓页新增跟随账户视图和完整交易流水变化的六维交易风格画像，展示可解释证据、样本量、置信度与限制项；样本不足时明确标记“形成中”，不把行为倾向包装为投资能力评分。
- [修复] 持仓明细的当日收益率改用现金流调整分母：以期初持仓市值加当日净新增持仓资金计算，避免盘中加仓后把新增仓位盈亏除以较小的期初市值而放大涨跌比例，同时保持卖出后回补的做 T 口径。
- [修复] Web 持仓页启用实时行情估值，并在页面可见时每 30 秒及窗口重新获得焦点时静默刷新快照，使现价、市值、持仓盈亏、当日盈亏与总权益联动更新；行情刷新失败时保留最后一次成功数据并显示降级提示。
- [新功能] Web 持仓页支持上传券商交易截图，经 Vision 模型识别后在可编辑预览表中逐笔核对，并由用户确认写入指定账户；重复提交同一截图记录会按来源哈希去重。
- [改进] 持仓看板和事件流水完整跟随账户视图切换，单账户明细隐藏冗余账户列，并增加证券名称、市值、持仓/可用、成本/现价、当日盈亏、个股仓位、持股天数、止盈止损、标签及回本涨幅等券商式字段。
- [修复] 持仓页快速切换账户时丢弃旧账户的迟到快照、风险和事件响应；全部账户的持仓盈亏与当日盈亏先按账户基准币种计算再统一折算，避免跨币种直接相加。
- [修复] 持仓快照新增连续持仓周期的摊薄成本、持仓盈亏与收益率；同日交易按买入后卖出回放，只有日终持仓为零才结束周期，避免做 T 卖出记录先录入时把回补仓位误判为全新仓位。
...
- [改进] Web 应用外壳移除桌面左侧固定导航，改为顶部悬浮命令栏与移动端底部 Dock；主题和语言收为右上角图标工具，低频入口进入“更多”菜单，并把业务内容画布扩展到 1760px，同时保持既有路由、AlphaSift 条件入口、退出确认和页面业务逻辑不变。
- [修复] 持仓看板继续使用“当日盈亏”名称，但账户级数值改为汇总当日全部持仓变动：当天已清仓股票的实际损益、新开仓及当前持仓的市值变动会按昨收和净交易现金流共同计算，避免清仓亏损因不再存在持仓行而从账户结果中遗漏。
- [修复] 自选股批量分析的 LiteLLM 阶段现在遵守全局生成并发上限，等待槽位不消耗单次模型超时预算；单只股票上游失败会标明具体标的并继续其他任务，批量提交提示也不再把“此前已运行”误写成当前运行总数。
